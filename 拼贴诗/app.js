...
const presetBackgrounds = {
  fiber: window.PRESET_BACKGROUND_DATA?.fiber || "./assets/canvas-paper-fiber.jpg",
...
async function exportCanvas() {
  try {
    ...
    strips.forEach((strip) => {
      ...
      const text = strip.dataset.text;
      ctx.font = "15px Songti SC, STSong, Georgia, serif";
      const width = Math.min(Math.max(ctx.measureText(text).width + 38, 86), rect.width * 0.82);
      const height = 42 + Math.max(0, Math.ceil(text.length / 14) - 1) * 20;
      ctx.save();
      ...
      drawWrappedText(ctx, text, -width / 2 + 12, -height / 2 + 10, width - 24, 20);
      ctx.restore();
    });
